...
function BeachCanvas() {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      // SHORELINE — waves washing onto sand
      for (let w = 0; w < 4; w++) {
      ...
      buildG.addColorStop(0, "#f8eedc");
      buildG.addColorStop(1, "#eedfc8");
      ctx!.fillStyle = buildG;
      ...
      for (let fl = 1; fl < 3; fl++) {
        ctx!.beginPath();
        ctx!.moveTo(hx, hy + hh * fl / 3);
        ctx!.lineTo(hx + hw, hy + hh * fl / 3);
      ...
      for (let row = 0; row < 3; row++) {
        for (let col = 0; col < 4; col++) {
          ...
          ctx!.strokeStyle = "#9a7848"; ctx!.lineWidth = 0.8;
      ...
      ctx!.beginPath();
      ctx!.moveTo(hx - W * 0.01, hy);
      ctx!.lineTo(hx + hw / 2, hy - hh * 0.18);
      ctx!.lineTo(hx + hw + W * 0.01, hy);
      ...
      sailboats.forEach(b => {
        b.x += b.speed;
        if (b.x > 0.56) b.x = 0.02;
        const bx = W * b.x;
        const by = seaTop + (H - seaTop) * 0.22 + Math.sin(t * 1.1 + b.x * 8) * H * 0.025;
        const s = H * b.sz * 0.11;
        ctx!.save(); ctx!.translate(bx, by);
        ctx!.beginPath();
        ctx!.moveTo(-s, 0); ctx!.lineTo(s, 0); ctx!.lineTo(s * 0.65, s * 0.45); ctx!.lineTo(-s * 0.65, s * 0.45);
        ctx!.closePath(); ctx!.fillStyle = "#c07030"; ctx!.fill();
        ctx!.strokeStyle = "#803010"; ctx!.lineWidth = 0.8; ctx!.stroke();
        ctx!.beginPath(); ctx!.moveTo(0, 0); ctx!.lineTo(0, -s * 2.2);
        ctx!.strokeStyle = "#604020"; ctx!.lineWidth = 1.2; ctx!.stroke();
        ctx!.beginPath(); ctx!.moveTo(0, 0); ctx!.lineTo(0, -s * 2.1); ctx!.lineTo(s * 1.2, -s * 0.15); ctx!.closePath();
        ctx!.fillStyle = "rgba(255,255,255,0.93)"; ctx!.fill();
        ctx!.beginPath(); ctx!.moveTo(0, 0); ctx!.lineTo(0, -s * 1.7); ctx!.lineTo(-s * 0.85, -s * 0.1); ctx!.closePath();
        ctx!.fillStyle = "rgba(240,240,255,0.85)"; ctx!.fill();
        ctx!.restore();
      });
      ...
      people.forEach(p => {
        p.x += p.speed;
        p.phase += 0.09;
        if (p.x > 0.96) p.x = 0.58;
        if (p.x < 0.55) p.x = 0.94;
        const px = W * p.x;
        const py = H * 0.82;
        const sz = H * 0.09;
        ...
        ctx!.beginPath(); ctx!.arc(0, -sz * 0.72, sz * 0.175, 0, Math.PI * 2); ctx!.fill();
        ctx!.fillStyle = "#5c3010";
        ctx!.beginPath(); ctx!.arc(0, -sz * 0.78, sz * 0.15, Math.PI, 0); ctx!.fill();
        ctx!.restore();
      });
      ...
      birds.forEach(b => {
        ...
        const fl = Math.sin(b.phase) * H * 0.026;
        const sz = H * 0.024;
        ctx!.save(); ctx!.translate(bx, by);
        if (b.dir < 0) ctx!.scale(-1, 1);
        ctx!.strokeStyle = "rgba(25,25,45,0.65)"; ctx!.lineWidth = 1.3; ctx!.lineCap = "round";
        ctx!.beginPath();
        ctx!.moveTo(-sz * 1.4, fl * 0.4);
        ctx!.quadraticCurveTo(0, -fl * 0.25, sz * 1.4, fl * 0.4);
        ctx!.stroke();
        ctx!.restore();
      });
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <header className="bg-white border-b">
        ...
        {/* ── SADECE BU DIV DEĞİŞTİ: position:relative + canvas arka plana eklendi ── */}
        <div
          className="container mx-auto px-2 py-3"
          style={{ position: "relative", overflow: "hidden", minHeight: "110px" }}
        >
          {/* Beach animasyonu — logonun altında, z-index:0 */}
          <BeachCanvas />

          {/* Logo ve sağ alan — aynen korundu, sadece z-index:1 eklendi */}
        ...
        {/* Navigation — HİÇBİR DEĞİŞİKLİK YOK */}
        <nav className="bg-[#2a2a2a] text-white">
          ...
          {mobileMenuOpen && (
            <div className="md:hidden bg-[#2a2a2a] absolute w-full left-0 z-50 text-center">
             ...
             <Link href="/contact" className="block px-4 py-3 hover:bg-[#3a3a3a]" onClick={() => setMobileMenuOpen(false)}>Location and Transportation</Link>
             <Link href="/check-in" className="block px-4 py-3 hover:bg-[#3a3a3a]" onClick={() => setMobileMenuOpen(false)}>Check-In Guide</Link>
             <Link href="/contact" className="block px-4 py-3 hover:bg-[#3a3a3a]" onClick={() => setMobileMenuOpen(false)}>Contack Us</Link>
            </div>
           )}
          <div className="container mx-auto px-4">
            <div className="hidden md:flex items-center justify-between">
              <div className="flex">
                <Link href="/" className="px-3 py-4 bg-[#1a1a1a] hover:bg-[#3a3a3a] transition text-sm lg:text-base">Home</Link>
                <Link href="/about" className="px-3 py-4 hover:bg-[#3a3a3a] transition text-sm lg:text-base">About us</Link>
                <Link href="/fethiye" className="px-3 py-4 hover:bg-[#3a3a3a] transition text-sm lg:text-base">Fethiye</Link>
                <Link href="/rooms" className="px-3 py-4 hover:bg-[#3a3a3a] transition flex items-center gap-2">Reserve Your Stay</Link>
                <Link href="/gallery" className="px-3 py-4 hover:bg-[#3a3a3a] transition text-sm lg:text-base">Rooms and Aparts</Link>
                <Link href="/contact" className="px-3 py-4 hover:bg-[#3a3a3a] transitiontext-sm lg:text-base">Location and Transportation</Link>
              </div>
              <div className="flex">
                <Link href="/check-in" className="px-3 py-4 bg-black hover:bg-[#3a3a3a] transitiontext-sm lg:text-base">Check-In Guide</Link>
                <Link href="/transfer" className="px-3 py-4 hover:bg-[#3a3a3a] transitiontext-sm lg:text-base">Airport Transfer</Link>
              </div>
            </div>
          </div>
        </nav>
        ...
        <div className="w-full h-[600px] relative overflow-hidden">
          ...
          <div className="absolute inset-0 bg-black bg-opacity-20 flex items-center justify-center">
            <div className="text-center text-white">
              <h2 className="text-6xl font-bold mb-4">Welcome to Paradise</h2>
              <p className="text-2xl">Experience the magic of Fethiye</p>
            </div>
          </div>
        </div>
      </section>

      {/* Reservation Form — DEĞİŞİKLİK YOK */}
      <section className="bg-gray-100 py-12">
        <div className="container mx-auto px-4 max-w-4xl">
          <h2 className="text-3xl font-semibold text-center mb-8">RESERVATION</h2>
          <div className="bg-white p-8 shadow-lg">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium mb-2">Check - In</label>
                <input type="date" className="w-full px-4 py-3 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-gray-400" placeholder="DD MM YYYY" />
              </div>
              <div>
            ...
            <button className="w-full mt-6 bg-black text-white py-4 text-lg font-medium hover:bg-gray-800 transition">
              SEARCH ROOM
            </button>
          </div>
        </div>
